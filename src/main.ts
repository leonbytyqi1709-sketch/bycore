...
// Supabase Cloud-Sync v2 + Sidebar User Block
// ============================================================
...
// AKZENTFARBEN SYSTEM
// ============================================================
const ACCENT_COLORS: Record<string, { color: string; glow: string; hover: string }> = {
  purple: { color: '#6C63FF', glow: 'rgba(108,99,255,0.30)', hover: '#5a52e0' },
  blue:   { color: '#3B9EFF', glow: 'rgba(59,158,255,0.30)',  hover: '#2b8ef0' },
  cyan:   { color: '#00D4FF', glow: 'rgba(0,212,255,0.30)',   hover: '#00bce0' },
  green:  { color: '#2ECC71', glow: 'rgba(46,204,113,0.30)',  hover: '#27b562' },
  lime:   { color: '#A3E635', glow: 'rgba(163,230,53,0.30)',  hover: '#8fd42a' },
  yellow: { color: '#FFC300', glow: 'rgba(255,195,0,0.30)',   hover: '#e6b000' },
  orange: { color: '#FF6B35', glow: 'rgba(255,107,53,0.30)',  hover: '#e85a25' },
  red:    { color: '#FF4757', glow: 'rgba(255,71,87,0.30)',   hover: '#e63546' },
  pink:   { color: '#FF6EB4', glow: 'rgba(255,110,180,0.30)', hover: '#e85ca0' },
  white:   { color: '#CCCCCC', glow: 'rgba(204,204,204,0.20)', hover: '#bbbbbb' },
  // ── Neue Farben ──────────────────────────────────────
  crimson: { color: '#9B1B30', glow: 'rgba(155,27,48,0.35)',   hover: '#7a1525' },
  navy:    { color: '#1A3A6B', glow: 'rgba(26,58,107,0.35)',   hover: '#142d55' },
  forest:  { color: '#1A6B3C', glow: 'rgba(26,107,60,0.35)',   hover: '#145530' },
  violet:  { color: '#7C3AED', glow: 'rgba(124,58,237,0.35)',  hover: '#6429d4' },
  gold:    { color: '#B8860B', glow: 'rgba(184,134,11,0.35)',  hover: '#9a7009' },
  teal:    { color: '#0F7173', glow: 'rgba(15,113,115,0.35)',  hover: '#0b5b5d' },
};

function applyAccent(colorKey: string) {
  const def = ACCENT_COLORS[colorKey] || ACCENT_COLORS['purple'];
  ...
  root.style.setProperty('--accent-subtle', def.glow.replace('0.30)', '0.08)'));
  root.style.setProperty('--border-accent', def.color);
  root.style.setProperty('--shadow-glow', `0 0 30px ${def.glow}`);
  document.body.dataset.accent = colorKey;
  localStorage.setItem('bycore-accent', colorKey);
  // Update active state in settings if open
  document.querySelectorAll('.accent-btn').forEach(btn => {
    btn.classList.toggle('active', (btn as HTMLElement).dataset.color === colorKey);
  });
}

function initAccent() {
  const saved = localStorage.getItem('bycore-accent') || 'orange';
  applyAccent(saved);
}
...
let notifDailyTimer: any = null;
let notifHourlyTimer: any = null;
...
async function initNotifications() {
  try {
    ...
    function scheduleDailyReminder() {
      ...
      clearTimeout(notifDailyTimer);
      notifDailyTimer = setTimeout(() => {
        sendDailyNotif(sendNotification);
        setInterval(() => sendDailyNotif(sendNotification), 24 * 60 * 60 * 1000);
      }, delay);
    }

    // Stündliche Überprüfung auf überfällige Tasks
    function scheduleHourlyCheck() {
      clearInterval(notifHourlyTimer);
      notifHourlyTimer = setInterval(() => {
        sendOverdueNotif(sendNotification);
      }, 60 * 60 * 1000);
      // Sofort prüfen beim Start (nach 10s damit Tasks geladen sind)
      setTimeout(() => sendOverdueNotif(sendNotification), 10_000);
    }
...
function sendDailyNotif(sendNotification: Function) {
  const todayStr = getTodayStr();
  const today = tasksCache.filter(t => t.due_date === todayStr && !t.done);
  const overdue = tasksCache.filter(t => t.due_date && t.due_date < todayStr && !t.done);
  const important = today.filter(t => t.priority === 'important');

  if (today.length === 0 && overdue.length === 0) return;

  let body = '';
  if (today.length > 0) body += `📋 ${today.length} Task(s) heute fällig`;
  if (important.length > 0) body += ` • 🔴 ${important.length} wichtig`;
  if (overdue.length > 0) body += `\n⚠️ ${overdue.length} überfällig`;

  sendNotification({ title: 'BYCORE – Guten Morgen!', body: body.trim() });
}

function sendOverdueNotif(sendNotification: Function) {
  const todayStr = getTodayStr();
  const overdue = tasksCache.filter(t => t.due_date && t.due_date < todayStr && !t.done);
  if (overdue.length === 0) return;
  const important = overdue.filter(t => t.priority === 'important');
  const body = important.length > 0
    ? `${overdue.length} Task(s) überfällig – ${important.length} davon 🔴 wichtig`
    : `${overdue.length} Task(s) sind überfällig!`;
  sendNotification({ title: '⚠️ BYCORE – Überfällige Tasks', body });
}

// ============================================================
// AUTH CHECK + SIDEBAR USER BLOCK
// ============================================================
...
function setupTitlebar() {
  try {
    ...
    // Fallback fuer Browser-Dev-Modus (localhost)
...
async function loadNotes(): Promise<Note[]> {
  ...
  notesCache = (data || []).map((n: any) => ({ ...n, pinned: n.pinned ?? false }));
  return notesCache;
}

async function saveNoteToSupabase(id: string, title: string, content: string) {
  await supabase.from('notes').update({ title, content }).eq('id', id);
  const note = notesCache.find(n => n.id === id);
  if (note) { note.title = title; note.content = content; }
}

async function createNoteInSupabase(): Promise<Note | null> {
  const userId = await getUserId();
  const { data, error } = await supabase
    .from('notes').insert({ user_id: userId, title: '', content: '', pinned: false }).select().single();
  if (error) { console.error(error); return null; }
  return { ...data, pinned: false };
}

async function deleteNoteFromSupabase(id: string) {
  await supabase.from('notes').delete().eq('id', id);
  notesCache = notesCache.filter(n => n.id !== id);
}

async function toggleNotePinInSupabase(id: string) {
  const note = notesCache.find(n => n.id === id);
  if (!note) return;
...
function parseMarkdown(text: string): string {
  ...
  html = html.replace(/<p>(<h[1-3]>)/g, "$1"); html = html.replace(/(<\/h[1-3]>)<\/p>/g, "$1");
  html = html.replace(/<p>(<pre>)/g, "$1"); html = html.replace(/(<\/pre>)<\/p>/g, "$1");
  ...
  html = html.replace(/<p>(<hr>)/g, "$1"); html = html.replace(/(<hr>)<\/p>/g, "$1");
  return html;
}

async function createNote() {
  const newNote = await createNoteInSupabase();
  if (!newNote) return;
  notesCache.unshift(newNote);
  activeNoteId = newNote.id;
  isPreviewMode = false;
  renderNotes();
  setTimeout(() => (document.getElementById("noteTitleInput") as HTMLInputElement)?.focus(), 50);
}

async function deleteNote(id: string) {
  await deleteNoteFromSupabase(id);
  activeNoteId = notesCache.length > 0 ? notesCache[0].id : null;
  isPreviewMode = false;
  renderNotes();
}

async function toggleNotePin(id: string) {
  await toggleNotePinInSupabase(id);
  renderNotes();
}
...
function saveCurrentNote() {
  ...
  if (titleInput && textarea) saveNoteToSupabase(activeNoteId, titleInput.value, textarea.value);
}
...
function renderNotes() {
    ...
    if (notes.length === 0) {
      ...
      listEl.innerHTML = sorted.map((note) => `
        <div class="note-item ${note.id === activeNoteId ? "active" : ""} ${note.pinned ? "pinned" : ""}" data-note-id="${note.id}">
          ...
          <div class="note-item-date">${new Date(note.updated_at).toLocaleDateString("de-DE", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" })}</div>
        </div>
      `).join("");
  ...
  if (searchInput && searchInput.value.trim()) applyNoteSearch(searchInput.value);

  const editorArea = document.getElementById("notesEditorArea");
  if (editorArea) {
    if (!activeNote) {
      editorArea.innerHTML = `<div class="notes-empty-state"><span class="empty-icon">📝</span><p>Wähle eine Notiz oder erstelle eine neue</p></div>`;
    } else {
...
function prioToDb(p: string): string { if (p === "important") return "high"; if (p === "normal") return "medium"; return "low"; }
function prioFromDb(p: string): Task["priority"] { if (p === "high") return "important"; if (p === "medium") return "normal"; return "optional"; }
function statusToDb(s: string): string { if (s === "progress") return "in_progress"; return s; }
function statusFromDb(s: string): Task["status"] { if (s === "in_progress") return "progress"; return s as Task["status"]; }
...
async function loadTasks(): Promise<Task[]> {
  ...
  if (error) { console.error(error); return []; }
  tasksCache = (data || []).map((t: any) => ({
    id: t.id, title: t.title, description: t.description || '',
    priority: prioFromDb(t.priority), status: statusFromDb(t.status),
    done: t.status === 'done', created_at: t.created_at,
...
async function createTask(title: string, description: string, priority: Task["priority"], dueDate: string) {
  const userId = await getUserId();
  const { data, error } = await supabase
    .from('tasks').insert({ user_id: userId, title, description, priority: prioToDb(priority), due_date: dueDate || null, status: 'todo' })
    .select().single();
  if (error) { console.error(error); return; }
  tasksCache.unshift({ id: data.id, title, description, priority, status: 'todo', done: false, created_at: data.created_at, due_date: dueDate });
  renderTasks();
}

async function updateTask(id: string, title: string, description: string, priority: Task["priority"], status: Task["status"], dueDate: string) {
  const { error } = await supabase.from('tasks').update({ title, description, priority: prioToDb(priority), status: statusToDb(status), due_date: dueDate || null }).eq('id', id);
  if (error) { console.error(error); return; }
  const task = tasksCache.find(t => t.id === id);
  if (task) { task.title = title; task.description = description; task.priority = priority; task.status = status; task.done = status === 'done'; task.due_date = dueDate; }
  renderTasks();
}
...
async function toggleTaskDone(id: string) {
  ...
  await supabase.from('tasks').update({ status: statusToDb(task.status) }).eq('id', id);
  renderTasks();
}

async function moveTask(id: string, newStatus: "todo" | "progress" | "done") {
  const task = tasksCache.find(t => t.id === id);
  if (!task) return;
  task.status = newStatus;
  task.done = newStatus === "done";
  await supabase.from('tasks').update({ status: statusToDb(newStatus) }).eq('id', id);
  renderTasks();
}
...
function getPriorityLabel(p: string): string { if (p === "important") return "🔴"; if (p === "normal") return "🟡"; return "⚪"; }
function getPriorityText(p: string): string { if (p === "important") return "Wichtig"; if (p === "normal") return "Normal"; return "Optional"; }

function showTaskModal(editId?: string) {
  editingTaskId = editId || null;
  const task = editId ? tasksCache.find((t) => t.id === editId) : null;
  ...
  overlay.innerHTML = `
    <div class="task-modal">
      ...
      <div class="task-modal-field">
        ...
        <input type="date" id="taskModalDue" value="${task?.due_date || ""}" />
      </div>
  ...
  document.getElementById("btnSaveTask")?.addEventListener("click", () => {
    ...
    if (!title) { (document.getElementById("taskModalTitle") as HTMLInputElement).classList.add("input-error"); return; }
    ...
    if (editingTaskId) { updateTask(editingTaskId, title, desc, priority, status, due); } else { createTask(title, desc, priority, due); }
    closeModal(overlay);
  });
      ...
      return taskList.map((t) => `
        <div class="task-card" data-task-id="${t.id}" draggable="true">
          ...
          <div class="task-card-meta">
            ${t.due_date ? `<span class="task-due-tag ${isOverdue(t.due_date) && !t.done ? "overdue" : ""}">📅 ${new Date(t.due_date).toLocaleDateString("de-DE")}</span>` : ""}
      ...
      container.innerHTML = `
        <div class="tasks-list-view">
          <div class="task-list-header-row">
            <span class="tl-col tl-check"></span><span class="tl-col tl-prio">Prio</span>
            <span class="tl-col tl-title">Titel</span><span class="tl-col tl-status">Status</span>
            <span class="tl-col tl-due">Fällig</span><span class="tl-col tl-actions">Aktionen</span>
          </div>
          ${filtered.map((t) => `
            <div class="task-list-item" data-task-id="${t.id}">
              ...
              <span class="tl-col tl-due ${isOverdue(t.due_date) && !t.done ? "overdue" : ""}">${t.due_date ? new Date(t.due_date).toLocaleDateString("de-DE") : "—"}</span>
...
function bindTaskEvents(container: HTMLElement) {
  container.querySelectorAll("[data-toggle-id]").forEach((el) => { el.addEventListener("click", (e) => { e.stopPropagation(); toggleTaskDone(el.getAttribute("data-toggle-id")!); }); });
  container.querySelectorAll("[data-delete-id]").forEach((el) => { el.addEventListener("click", (e) => { e.stopPropagation(); if (confirm("Task löschen?")) deleteTask(el.getAttribute("data-delete-id")!); }); });
  container.querySelectorAll("[data-edit-id]").forEach((el) => { el.addEventListener("click", (e) => { e.stopPropagation(); showTaskModal(el.getAttribute("data-edit-id")!); }); });
  container.querySelectorAll("[data-move-progress]").forEach((el) => { el.addEventListener("click", (e) => { e.stopPropagation(); moveTask(el.getAttribute("data-move-progress")!, "progress"); }); });
  container.querySelectorAll("[data-move-done]").forEach((el) => { el.addEventListener("click", (e) => { e.stopPropagation(); moveTask(el.getAttribute("data-move-done")!, "done"); }); });
  container.querySelectorAll("[data-move-todo]").forEach((el) => { el.addEventListener("click", (e) => { e.stopPropagation(); moveTask(el.getAttribute("data-move-todo")!, "todo"); }); });
  container.querySelectorAll("[data-status-id]").forEach((el) => { el.addEventListener("change", (e) => { const select = e.target as HTMLSelectElement; moveTask(el.getAttribute("data-status-id")!, select.value as Task["status"]); }); });
}

function setupKanbanDragDrop() {
  const cards = document.querySelectorAll<HTMLElement>(".task-card[draggable]");
  const columns = document.querySelectorAll<HTMLElement>(".kanban-tasks");
  ...
  function getInsertPosition(col: HTMLElement, clientY: number): HTMLElement | null {
    const cards = [...col.querySelectorAll<HTMLElement>('.task-card:not(.dragging)')];
  ...
  columns.forEach((col) => {
    col.addEventListener("dragover", (e) => {
      ...
      // Smart placeholder insertion
      const insertBefore = getInsertPosition(col, (e as DragEvent).clientY);
      if (!placeholder) placeholder = createPlaceholder();
      if (insertBefore) {
        col.insertBefore(placeholder, insertBefore);
      } else {
...
function updateDashboardTaskCount() {
  const imp = tasksCache.filter((t) => t.priority === "important" && !t.done).length;
  const norm = tasksCache.filter((t) => t.priority === "normal" && !t.done).length;
  const opt = tasksCache.filter((t) => t.priority === "optional" && !t.done).length;
...
function setupTasksEvents() {
  ...
  document.getElementById("btnKanban")?.addEventListener("click", () => { taskView = "kanban"; document.getElementById("btnKanban")?.classList.add("active"); document.getElementById("btnList")?.classList.remove("active"); renderTasks(); });
  document.getElementById("btnList")?.addEventListener("click", () => { taskView = "list"; document.getElementById("btnList")?.classList.add("active"); document.getElementById("btnKanban")?.classList.remove("active"); renderTasks(); });
  document.querySelectorAll(".filter-tab").forEach((tab) => { tab.addEventListener("click", () => { document.querySelectorAll(".filter-tab").forEach((t) => t.classList.remove("active")); tab.classList.add("active"); taskFilter = (tab.getAttribute("data-filter") || "all") as any; renderTasks(); }); });
}
...
interface CalendarEvent {
  ...
  startTime: string; endTime: string; color: string; created_at: string;
}
...
async function loadEvents(): Promise<CalendarEvent[]> {
  ...
  if (error) { console.error(error); return []; }
  eventsCache = (data || []).map((e: any) => ({
    id: e.id, title: e.title, description: e.description || '',
    date: e.start_time ? e.start_time.slice(0, 10) : '',
    startTime: e.start_time ? e.start_time.slice(11, 16) : '',
    endTime: e.end_time ? e.end_time.slice(11, 16) : '',
    color: e.color || 'blue', created_at: e.created_at,
  }));
  return eventsCache;
}
...
function getMonthName(month: number): string { return ["Januar","Februar","März","April","Mai","Juni","Juli","August","September","Oktober","November","Dezember"][month]; }
function toDateStr(y: number, m: number, d: number): string { return `${y}-${String(m + 1).padStart(2, "0")}-${String(d).padStart(2, "0")}`; }
function getTodayStr(): string { const t = new Date(); return toDateStr(t.getFullYear(), t.getMonth(), t.getDate()); }
...
function getCalendarDays(year: number, month: number): { date: string; day: number; currentMonth: boolean }[] {
  ...
  for (let i = startOffset - 1; i >= 0; i--) { const d = daysInPrevMonth - i; const pm = month === 0 ? 11 : month - 1; const py = month === 0 ? year - 1 : year; days.push({ date: toDateStr(py, pm, d), day: d, currentMonth: false }); }
  for (let d = 1; d <= daysInMonth; d++) days.push({ date: toDateStr(year, month, d), day: d, currentMonth: true });
  const remaining = 42 - days.length;
  for (let d = 1; d <= remaining; d++) { const nm = month === 11 ? 0 : month + 1; const ny = month === 11 ? year + 1 : year; days.push({ date: toDateStr(ny, nm, d), day: d, currentMonth: false }); }
  return days;
}
...
function renderCalendar() {
  ...
  container.innerHTML = `
    <div class="calendar-weekdays">${["Mo","Di","Mi","Do","Fr","Sa","So"].map((w) => `<div class="calendar-weekday">${w}</div>`).join("")}</div>
    <div class="calendar-days">
      ${days.map((d) => {
        const dayEvents = events.filter((e) => e.date === d.date);
        const cls = ["calendar-day", !d.currentMonth ? "other-month" : "", d.date === today ? "today" : "", d.date === selectedDate ? "selected" : ""].filter(Boolean).join(" ");
        return `<div class="${cls}" data-date="${d.date}"><span class="day-number">${d.day}</span><div class="day-events">${dayEvents.slice(0, 3).map((e) => `<div class="day-event-dot color-${e.color}">${e.startTime ? e.startTime + " " : ""}${e.title}</div>`).join("")}${dayEvents.length > 3 ? `<div class="day-event-more">+${dayEvents.length - 3} mehr</div>` : ""}</div></div>`;
      }).join("")}
    </div>
  `;
  container.querySelectorAll(".calendar-day").forEach((el) => { el.addEventListener("click", () => { selectedDate = el.getAttribute("data-date"); renderCalendar(); }); });
  ...
  if (sidebarEvents && selectedDate) {
    ...
    if (dayEvents.length === 0) {
      ...
      sidebarEvents.querySelectorAll("[data-edit-event]").forEach((el) => { el.addEventListener("click", () => showEventModal(el.getAttribute("data-edit-event")!)); });
      sidebarEvents.querySelectorAll("[data-delete-event]").forEach((el) => { el.addEventListener("click", () => { if (confirm("Termin löschen?")) deleteCalEvent(el.getAttribute("data-delete-event")!); }); });
    }
  }
  ...
  if (upcomingEl) {
    ...
    upcomingEl.innerHTML = `<div class="upcoming-title">Nächste Termine</div>${upcoming.length === 0 ? '<p style="font-size:12px;color:var(--text-secondary)">Keine anstehenden Termine</p>' : upcoming.map((e) => `<div class="upcoming-item"><span class="upcoming-item-dot" style="background:${getColorVal(e.color)}"></span><span class="upcoming-item-date">${new Date(e.date+"T00:00:00").toLocaleDateString("de-DE",{day:"2-digit",month:"2-digit"})}</span><span class="upcoming-item-title">${e.title}</span></div>`).join("")}`;
  }
}
...
async function createCalEvent(title: string, desc: string, date: string, start: string, end: string, color: string) {
  const userId = await getUserId();
  const startTime = date && start ? `${date}T${start}:00` : null;
  const endTime = date && end ? `${date}T${end}:00` : null;
  const { data, error } = await supabase.from('events').insert({ user_id: userId, title, description: desc, start_time: startTime, end_time: endTime, color }).select().single();
  if (error) { console.error(error); return; }
  eventsCache.push({ id: data.id, title, description: desc, date, startTime: start, endTime: end, color, created_at: data.created_at });
  renderCalendar();
}

async function updateCalEvent(id: string, title: string, desc: string, date: string, start: string, end: string, color: string) {
  const startTime = date && start ? `${date}T${start}:00` : null;
  const endTime = date && end ? `${date}T${end}:00` : null;
  await supabase.from('events').update({ title, description: desc, start_time: startTime, end_time: endTime, color }).eq('id', id);
  const ev = eventsCache.find(e => e.id === id);
  if (ev) { ev.title = title; ev.description = desc; ev.date = date; ev.startTime = start; ev.endTime = end; ev.color = color; }
  renderCalendar();
}
...
function showEventModal(editId?: string) {
  ...
  const ev = editId ? eventsCache.find((e) => e.id === editId) : null;
  ...
  overlay.innerHTML = `
    <div class="event-modal">
      <div class="event-modal-top"><h2>${ev ? "Termin bearbeiten" : "Neuer Termin"}</h2><button class="btn-modal-close" id="btnCloseEvent">✕</button></div>
      ...
      <div class="event-modal-field">
        <label>Farbe</label>
        <div class="color-picker-row">${["blue","green","red","orange","purple"].map((c) => `<div class="color-option color-${c} ${defColor === c ? "selected" : ""}" data-color="${c}"></div>`).join("")}</div>
      </div>
  ...
  overlay.querySelectorAll(".color-option").forEach((o) => { o.addEventListener("click", () => { overlay.querySelectorAll(".color-option").forEach((x) => x.classList.remove("selected")); o.classList.add("selected"); selColor = o.getAttribute("data-color") || "blue"; }); });
  ...
  document.getElementById("btnSaveEv")?.addEventListener("click", () => {
    ...
    if (editingEventId) { updateCalEvent(editingEventId, title, desc, date, start, end, selColor); } else { createCalEvent(title, desc, date, start, end, selColor); }
    selectedDate = date; close();
  });
...
function getAppStats() {
  return {
    totalNotes: notesCache.length, pinnedNotes: notesCache.filter((n) => n.pinned).length,
    totalTasks: tasksCache.length, openTasks: tasksCache.filter((t) => !t.done).length,
    doneTasks: tasksCache.filter((t) => t.done).length, totalEvents: eventsCache.length,
    upcomingEvents: eventsCache.filter((e) => e.date >= getTodayStr()).length,
  };
}
...
if ((window as any).electronAPI?.onSystemStats) {
  (window as any).electronAPI.onSystemStats((stats: any) => { lastSystemStats = stats; updateSystemMonitorUI(); });
}

function updateSystemMonitorUI() {
  if (!lastSystemStats) return;
  const statusEl = document.querySelector(".sys-status");
  if (statusEl) { statusEl.textContent = lastSystemStats.network.online ? "Online" : "Offline"; statusEl.className = `sys-hero-number sys-status ${lastSystemStats.network.online ? "online" : "offline"}`; }
  const cpuEl = document.getElementById("sysCpuValue"); if (cpuEl) cpuEl.textContent = lastSystemStats.cpu + "%";
  const ramEl = document.getElementById("sysRamValue"); if (ramEl) ramEl.textContent = `${lastSystemStats.ram.used} / ${lastSystemStats.ram.total} GB`;
  const diskUsedEl = document.getElementById("sysDiskUsed"); if (diskUsedEl) diskUsedEl.textContent = lastSystemStats.disk.used + " GB";
  const diskTotalEl = document.getElementById("sysDiskTotal"); if (diskTotalEl) diskTotalEl.textContent = lastSystemStats.disk.total + " GB";
  const netSpeedEl = document.getElementById("sysNetSpeed"); if (netSpeedEl) netSpeedEl.textContent = lastSystemStats.network.speed;
  const ringFill = document.querySelector(".sys-ring-fill") as any;
  const ringPercent = document.querySelector(".sys-ring-percent");
  if (ringFill && ringPercent && lastSystemStats.disk.total > 0) { const percent = ((lastSystemStats.disk.used / lastSystemStats.disk.total) * 100); ringPercent.textContent = percent.toFixed(1) + "%"; ringFill.setAttribute("stroke-dasharray", `${percent * 2.64} 264`); }
}
...
function renderSystemMonitor() {
  ...
  const userName = localStorage.getItem("bycore-username") || "User";
  ...
  container.innerHTML = `
    <div class="sys-hero">
      ...
      <div class="sys-hero-right">
        ...
        <div class="sys-hero-stat"><span class="sys-hero-number">Supabase ☁️</span><span class="sys-hero-label">Cloud Sync</span></div>
    ...
    <div class="sys-grid">
      <div class="sys-card sys-card-accent">
        <div class="sys-card-icon">🖥️</div><h3>System Hardware</h3>
        <div class="sys-rows">
          <div class="sys-row"><span class="sys-label">CPU</span><span class="sys-value" id="sysCpuValue">${lastSystemStats?.cpu ?? "..."}%</span></div>
          <div class="sys-row"><span class="sys-label">RAM</span><span class="sys-value" id="sysRamValue">${lastSystemStats ? `${lastSystemStats.ram.used} / ${lastSystemStats.ram.total} GB` : "..."}</span></div>
      ...
      <div class="sys-card">
        <div class="sys-card-icon">💾</div><h3>Laufwerk (Disk)</h3>
      ...
      <div class="sys-card">
        <div class="sys-card-icon">⚙️</div><h3>Konfiguration</h3>
        <div class="sys-rows">
  ...
  systemInterval = setInterval(() => {
    ...
    if (uptimeEl) { const sec = Math.floor((Date.now() - startTime) / 1000); const min = Math.floor(sec / 60); const hr = Math.floor(min / 60); uptimeEl.textContent = hr > 0 ? `${hr}h ${min % 60}m` : min > 0 ? `${min}m ${sec % 60}s` : `${sec}s`; }
  }, 1000);
}

function setupSystemMonitor() { renderSystemMonitor(); }
...
function renderSettings() {
    ...
    <div class="settings-grid">
      ...
      <div class="settings-card">
        ...
        <div class="settings-field" style="margin-top:16px">
          ...
          <p class="settings-hint">Wähle eine Farbe für Buttons, Highlights und Glow-Effekte</p>
          <div class="accent-grid" id="accentGrid">
            ${Object.entries(ACCENT_COLORS).map(([key, val]) => `
              <button class="accent-btn ${(localStorage.getItem('bycore-accent') || 'orange') === key ? 'active' : ''}"
                data-color="${key}" title="${key}"
                style="background:${val.color};box-shadow:${(localStorage.getItem('bycore-accent') || 'orange') === key ? `0 0 10px ${val.glow}` : 'none'}">
              </button>
            `).join('')}
          </div>
        </div>
      ...
      <div class="settings-card">
        <div class="settings-card-header"><h3>🔔 Benachrichtigungen</h3></div>
        <div class="settings-field">
          <label>Push-Notifications</label>
          <p class="settings-hint">Tägliche Erinnerung um 09:00 Uhr + stündliche Warnung bei überfälligen Tasks</p>
          <button class="settings-btn" id="btnTestNotif">🔔 Test-Benachrichtigung senden</button>
        </div>
      </div>
      <div class="settings-card">
        <div class="settings-card-header"><h3>☁️ Cloud Sync</h3></div>
        <div class="settings-field">
          <label>Status</label>
          <p class="settings-hint">Daten werden automatisch mit Supabase synchronisiert</p>
          <div style="display:inline-block;padding:6px 12px;border-radius:6px;font-size:13px;background:rgba(74,222,128,0.1);border:1px solid rgba(74,222,128,0.3);color:#4ade80;">✅ Supabase aktiv</div>
        </div>
        <div class="settings-field" style="margin-top:16px">
          <label>Abmelden</label>
          <p class="settings-hint">Aus dem aktuellen Account ausloggen</p>
          <button class="settings-btn settings-btn-danger" id="btnLogout">🚪 Abmelden</button>
        </div>
      </div>
      <div class="settings-card settings-card-danger">
        ...
        <div class="settings-field">
          ...
          <p class="settings-hint">Löscht alle Notizen, Tasks und Termine aus der Cloud unwiderruflich</p>
  ...
  document.getElementById("btnSaveUsername")?.addEventListener("click", () => {
    ...
    if (name) { localStorage.setItem("bycore-username", name); input.classList.add("settings-input-success"); setTimeout(() => input.classList.remove("settings-input-success"), 1500); }
  });
  document.getElementById("btnThemeDark")?.addEventListener("click", () => { isDark = true; document.documentElement.setAttribute("data-theme", "dark"); localStorage.setItem("bycore-theme", "dark"); if (themeToggle) themeToggle.textContent = "🌙"; renderSettings(); });
  document.getElementById("btnThemeLight")?.addEventListener("click", () => { isDark = false; document.documentElement.setAttribute("data-theme", "light"); localStorage.setItem("bycore-theme", "light"); if (themeToggle) themeToggle.textContent = "☀️"; renderSettings(); });

  // Akzentfarben
  document.getElementById("accentGrid")?.addEventListener("click", (e) => {
    const btn = (e.target as HTMLElement).closest(".accent-btn") as HTMLElement;
    if (!btn) return;
    const color = btn.dataset.color!;
    applyAccent(color);
    // Update visuals in grid immediately
    document.querySelectorAll(".accent-btn").forEach((b: any) => {
      const isActive = b.dataset.color === color;
      b.classList.toggle("active", isActive);
      b.style.boxShadow = isActive ? `0 0 12px ${ACCENT_COLORS[b.dataset.color]?.glow}` : "none";
    });
  });
  ...
  document.getElementById("btnTestNotif")?.addEventListener("click", async () => {
    try {
      ...
      if (granted) {
        const overdue = tasksCache.filter(t => t.due_date && t.due_date < getTodayStr() && !t.done).length;
        const today = tasksCache.filter(t => t.due_date === getTodayStr() && !t.done).length;
        sendNotification({
          title: "BYCORE – Test ✅",
          body: `${today} Task(s) heute · ${overdue} überfällig · Notifications aktiv!`
        });
      }
    } catch (_) {
      alert("ℹ️ Benachrichtigungen nur in der Tauri-App verfügbar (nicht im Browser).");
    }
  });
  document.getElementById("btnLogout")?.addEventListener("click", async () => {
    if (confirm("Wirklich abmelden?")) { await supabase.auth.signOut(); window.location.href = "login.html"; }
  });
  document.getElementById("btnResetAll")?.addEventListener("click", async () => {
    if (confirm("⚠️ ACHTUNG: Alle BYCORE-Daten werden unwiderruflich aus der Cloud gelöscht!\n\nBist du sicher?")) {
      if (confirm("Wirklich ALLES löschen? Das kann nicht rückgängig gemacht werden!")) {
        const userId = await getUserId();
        await supabase.from('notes').delete().eq('user_id', userId);
        await supabase.from('tasks').delete().eq('user_id', userId);
        await supabase.from('events').delete().eq('user_id', userId);
        notesCache = []; tasksCache = []; eventsCache = [];
        alert("✅ Alle Daten gelöscht."); location.reload();
      }
    }
  });
}

function setupSettings() { renderSettings(); }
...
function getUpcomingDashEvents(): string {
  const today = getTodayStr();
  const upcoming = eventsCache.filter((e) => e.date >= today).sort((a, b) => a.date.localeCompare(b.date) || a.startTime.localeCompare(b.startTime)).slice(0, 4);
  if (upcoming.length === 0) return '<div class="event-item" style="opacity:0.5"><span class="event-time">—</span><span class="event-title">Keine anstehenden Termine</span></div>';
...
function getDashboardHTML(): string {
  ...
  const norm = tasksCache.filter((t) => t.priority === "normal" && !t.done).length;
  const opt = tasksCache.filter((t) => t.priority === "optional" && !t.done).length;
  const totalDone = tasksCache.filter((t) => t.done).length;
  ...
  const todayEvents = eventsCache.filter((e) => e.date === getTodayStr()).length;
  const pinnedNotes = notesCache.filter((n) => n.pinned).length;
  ...
  return `
    <div class="dashboard">
      <div class="dash-hero">
        <div class="dash-hero-particles"><span class="particle"></span><span class="particle"></span><span class="particle"></span><span class="particle"></span><span class="particle"></span><span class="particle"></span></div>
        <div class="dash-hero-left">
          <p class="dash-hero-greeting">${greeting},</p>
          <h1 class="dash-hero-name">${userName} 👋</h1>
          <p class="dash-hero-date" id="date-display"></p>
        </div>
        <div class="dash-hero-center"><p class="clock" id="clock"></p></div>
        <div class="dash-hero-right">
          <div class="dash-hero-ring">
            <svg viewBox="0 0 100 100">
              <circle cx="50" cy="50" r="40" fill="none" stroke="rgba(255,255,255,0.1)" stroke-width="6"/>
              <circle cx="50" cy="50" r="40" fill="none" stroke="#fff" stroke-width="6" stroke-linecap="round" stroke-dasharray="${strokeDash} 251.2" transform="rotate(-90 50 50)" class="dash-ring-fill"/>
            </svg>
            <div class="dash-ring-text"><span class="dash-ring-num">${completionPct}%</span></div>
          </div>
          <span class="dash-ring-label">Produktivität</span>
        </div>
      </div>
      <div class="dash-stats-bar">
        <div class="dash-stat-chip"><span class="dash-chip-icon">📋</span><span class="dash-chip-num">${imp + norm + opt}</span><span class="dash-chip-label">Offene Tasks</span></div>
        <div class="dash-stat-chip"><span class="dash-chip-icon">✅</span><span class="dash-chip-num">${totalDone}</span><span class="dash-chip-label">Erledigt</span></div>
        <div class="dash-stat-chip"><span class="dash-chip-icon">📝</span><span class="dash-chip-num">${totalNotes}</span><span class="dash-chip-label">Notizen</span></div>
        <div class="dash-stat-chip"><span class="dash-chip-icon">📌</span><span class="dash-chip-num">${pinnedNotes}</span><span class="dash-chip-label">Gepinnt</span></div>
        <div class="dash-stat-chip"><span class="dash-chip-icon">📅</span><span class="dash-chip-num">${todayEvents}</span><span class="dash-chip-label">Heute</span></div>
      </div>
      <div class="dashboard-grid">
        ...
        <div class="dash-card dash-card-events">
          ...
          <div class="event-list">${getUpcomingDashEvents()}</div>
        </div>
...
const moduleTemplates: Record<string, string> = {
  notes: `<div class="notes-container"><div class="notes-sidebar"><div class="notes-sidebar-header"><h2>📝 Notizen</h2><div class="notes-sidebar-actions"><input type="text" class="notes-search" id="notesSearch" placeholder="Suchen..." /><button class="btn-new-note" id="btnNewNote">+ Neu</button></div></div><div class="notes-list" id="notesList"></div></div><div class="notes-editor-area" id="notesEditorArea"></div></div>`,
  tasks: `<div class="tasks-container"><div class="tasks-header"><h1>✅ Task Manager</h1><div class="tasks-header-actions"><button class="btn-view-toggle active" id="btnKanban">Kanban</button><button class="btn-view-toggle" id="btnList">Liste</button><button class="btn-new-task" id="btnNewTask">+ Neuer Task</button></div></div><div class="tasks-filters"><button class="filter-tab active" data-filter="all">Alle</button><button class="filter-tab" data-filter="important">🔴 Wichtig</button><button class="filter-tab" data-filter="normal">🟡 Normal</button><button class="filter-tab" data-filter="optional">⚪ Optional</button></div><div id="tasksContent"></div></div>`,
  calendar: `<div class="calendar-container"><div class="calendar-main"><div class="calendar-header"><h1>📅 Kalender</h1><div class="calendar-nav"><button class="btn-cal-nav" id="btnPrevMonth">◀</button><span class="calendar-month-label" id="calMonthLabel"></span><button class="btn-cal-nav" id="btnNextMonth">▶</button><button class="btn-cal-today" id="btnCalToday">Heute</button></div><div class="calendar-header-actions"><button class="btn-new-event" id="btnNewEvent">+ Neuer Termin</button></div></div><div class="calendar-grid" id="calendarGrid"></div></div><div class="calendar-sidebar"><div class="calendar-sidebar-header" id="calendarSidebarHeader"></div><div class="calendar-sidebar-events" id="calendarSidebarEvents"></div><div class="calendar-sidebar-upcoming" id="calendarUpcoming"></div></div></div>`,
  system: `<div class="system-container"><div class="system-header"><h1>📊 System Monitor</h1></div><div class="system-content" id="systemContent"></div></div>`,
  settings: `<div class="settings-container"><div class="settings-header"><h1>⚙️ Settings</h1></div><div class="settings-content" id="settingsContent"></div></div>`,
};
...
function setupScrollReveal() {
  ...
  document.querySelectorAll(".scroll-reveal, .scroll-reveal-left, .scroll-reveal-right, .scroll-reveal-scale, .scroll-reveal-flip").forEach((el) => scrollObserver!.observe(el));
}

function setup3DTilt() {
  document.querySelectorAll<HTMLElement>(".dash-card, .sys-card, .settings-card, .sys-stat-pill").forEach((card) => {
    let raf: number;
    card.addEventListener("mouseenter", () => { card.style.transition = "transform 0.1s ease-out, box-shadow 0.15s ease-out"; });
...
function applyModuleAnimations() {
  document.querySelectorAll(".dash-card").forEach((el, i) => { el.classList.add("scroll-reveal"); if (i < 6) el.classList.add(`delay-${i + 1}`); });
  const sysHero = document.querySelector(".sys-hero"); if (sysHero) sysHero.classList.add("scroll-reveal-scale");
  document.querySelectorAll(".sys-stat-pill").forEach((el, i) => { el.classList.add("scroll-reveal-scale"); if (i < 8) el.classList.add(`delay-${i + 1}`); });
  document.querySelectorAll(".sys-card").forEach((el, i) => { el.classList.add("scroll-reveal-flip"); if (i < 6) el.classList.add(`delay-${i + 1}`); });
  document.querySelectorAll(".settings-card").forEach((el, i) => { el.classList.add("scroll-reveal"); if (i < 6) el.classList.add(`delay-${i + 1}`); });
  document.querySelectorAll(".kanban-column").forEach((el, i) => { el.classList.add("scroll-reveal"); if (i < 3) el.classList.add(`delay-${i + 1}`); });
  document.querySelectorAll(".sidebar-event-item").forEach((el, i) => { el.classList.add("scroll-reveal"); if (i < 6) el.classList.add(`delay-${i + 1}`); });
  document.querySelectorAll(".note-item").forEach((el, i) => { el.classList.add("scroll-reveal"); if (i < 6) el.classList.add(`delay-${i + 1}`); });
  requestAnimationFrame(() => { setupScrollReveal(); setup3DTilt(); });
}
...
async function loadModule(moduleName: string) {
  ...
  if (moduleName === "notes") { document.getElementById("btnNewNote")?.addEventListener("click", createNote); renderNotes(); setupNotesSearch(); }
  if (moduleName === "tasks") { setupTasksEvents(); renderTasks(); }
...
navItems.forEach((item) => {
  item.addEventListener("click", () => { const target = item.getAttribute("data-module"); if (target) loadModule(target); });
});
...
if (!isDark) { document.documentElement.setAttribute("data-theme", "light"); if (themeToggle) themeToggle.textContent = "☀️"; }
...
function updateClock() {
  ...
  if (clockEl) clockEl.textContent = new Date().toLocaleTimeString("de-DE", { hour: "2-digit", minute: "2-digit", second: "2-digit" });
  if (dateEl) dateEl.textContent = new Date().toLocaleDateString("de-DE", { weekday: "long", year: "numeric", month: "long", day: "numeric" });
}
